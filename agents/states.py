...
class File(BaseModel):
    ...
    purpose: str =Field(
        description= "The purpose of the file, e.g., 'main applicatio logic', 'data processing module', etc."
    )

class Plan(BaseModel):
    name: str = Field(description= "The name of app to be built")
    description: str = Field(
        description= "A one line description of the app to be built, e.g. 'A web application for mananging personal finances.'"
    )
    teckstack: str = Field(
...
class CoderState(BaseModel):
    ...
    current_step_idx: int = Field(description="Th index of the current step in the implementation steps")
